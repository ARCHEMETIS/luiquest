...
~~12. ล้าง DB เริ่มนับศูนย์~~ ✅ **ล้างแล้ว 4 ส.ค.**

```
roadmaps / phases / daily_quests / quest_checklist_items
quest_completions / chat_messages / xp_awards
referrals / payments / activity_log ................. 0 ทั้งหมด
topics ....................... 6    (เนื้อหา เก็บไว้)
starter_quests ............... 126  (คลังเควส เก็บไว้)
public_stats: registered 2 · activated 0 · quests 0 · streak 0
```

เก็บไว้: 2 บัญชี (เจ้าของ + เพื่อน) พร้อม `referral_code` เดิม — **ไม่ลบบัญชี** เพราะ `handle_new_user` ยิงตอนสมัครครั้งแรกเท่านั้น ลบ profile ทิ้งแล้ว auth.users จะค้างไม่มี profile คู่กัน. ตัวเลข engagement เป็นศูนย์หมดแล้ว ถ้าอยากได้ `registered_total = 0` จริง ๆ ต้องลบ user ใน Supabase Auth เอง แล้วสมัครใหม่ + ตั้ง admin ใหม่

### 🚨 เจอตอนล้าง: `is_admin` เป็น `false` ทั้ง 2 บัญชี

บันทึกเก่าเข้าใจผิดว่า bootstrap admin ทำไปแล้ว — ความจริงคือ**ไม่มีใครเป็น admin เลย** ซึ่งแปลว่า `admin-payments` และ `verify-payment` จะตอบ **403 ให้ทุกคน** = **ขายพรีเมียมได้แต่อนุมัติสลิปไม่ได้ เงินค้างทั้งหมด**
→ **แก้แล้ว: ตั้ง `is_admin=true` ให้บัญชีเจ้าของ** (`d949d860-a444-4527-abfd-5809e68bdc9a`, Mahathi Awaetaloh)

13. **ประกาศ** 🚀 ← เหลือแค่นี้

**หลังประกาศ:** เฝ้าโควตา Gemini, เช็คสลิปรอตรวจในหน้า admin, เก็บ screenshot ตัวเลขไว้ทำสไลด์ (นำเสนอ 21 ส.ค.)

**ยังไม่ได้ทดสอบบนมือถือจริง** — โดยเฉพาะ 2 เรื่อง: PWA ติดตั้ง (add to home screen / standalone / ไอคอน) และมาสคอตที่เดินทับพื้นที่ checklist บนจอแคบ
