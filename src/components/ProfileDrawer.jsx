...
const ICON_PATHS = {
  ...
  // user-circle (Heroicons 24/outline) — ทางเข้าหน้าแก้ไขโปรไฟล์
  user: [
  ...
  // chat-bubble-left-right (Heroicons 24/outline) — ทางเข้าหน้าส่งความเห็น
  chat: [
        ...
        <>
          ...
          >
            ...
            <div className="min-h-0 flex-1 overflow-y-auto px-3.5 pb-3.5 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              ...
              {/* ทางเข้าหน้าแก้ไขโปรไฟล์ — ชื่อที่ขึ้นกระดานอันดับเดิมมาจาก Google แก้ไม่ได้เลย ต้องมีทางเข้าจริงสักที่ */}
              <button
                type="button"
                onClick={() => {
                  requestClose();
                ...
                className="mt-2.5 flex w-full items-center gap-2.5 rounded-2xl border-2 border-[#FBCFE8] bg-white/70 px-2.5 py-2 text-left transition hover:-translate-y-0.5 hover:border-[#F9A8D4] hover:bg-white active:translate-y-px"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-violet-100 text-violet-600">
                  <Icon paths={ICON_PATHS.user} className="h-4 w-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-[12px] font-bold text-[#831843]">แก้ไขโปรไฟล์</span>
                  <span className="block text-[9px] text-[#9D5C7C]">ชื่อ รูป และการแสดงบนกระดานอันดับ</span>
                </span>
                <Icon paths={ICON_PATHS.chevronRight} className="h-3.5 w-3.5 shrink-0 text-[#9D5C7C]" />
              </button>

              {/* ทางเข้าหน้าส่งความเห็น — ต้องมีทางเข้าถาวรที่หาเจอ ไม่ใช่โผล่ครั้งเดียวแล้วหาย */}
              <button
                type="button"
                onClick={() => {
                  requestClose();
                ...
                className="mt-2 flex w-full items-center gap-2.5 rounded-2xl border-2 border-[#FBCFE8] bg-white/70 px-2.5 py-2 text-left transition hover:-translate-y-0.5 hover:border-[#F9A8D4] hover:bg-white active:translate-y-px"
              >
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-pink-100 text-pink-600">
                  <Icon paths={ICON_PATHS.chat} className="h-4 w-4" />
                </span>
                <span className="min-w-0 flex-1">
                  <span className="block text-[12px] font-bold text-[#831843]">บอกความเห็นหน่อย</span>
                  <span className="block text-[9px] text-[#9D5C7C]">ติชม เจอบั๊ก หรืออยากให้มีอะไรเพิ่ม</span>
                </span>
                <Icon paths={ICON_PATHS.chevronRight} className="h-3.5 w-3.5 shrink-0 text-[#9D5C7C]" />
              </button>
                ...
                {showCapUpsell && (
                  ...
                  >
                    ...
                    <p className="mt-1 text-[#9D5C7C]">
                      ...
                      อยากเก็บหัวข้อไว้สลับเรียนต่อแบบไม่จำกัด กดปุ่ม "อัปเกรดพรีเมียม" ด้านบนได้เลย
                    </p>
                  </div>
                )}
              </div>
